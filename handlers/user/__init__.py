...
from handlers.user.start import bot_start, name_defined, character_defined, \
...
def register_user_handler(dp: Dispatcher):
    ...
    dp.register_message_handler(name_defined, state=UserInfo.waiting_for_name)
    dp.register_message_handler(grade_defined, state=UserInfo.waiting_for_grade)
    dp.register_message_handler(character_defined, state=UserInfo.waiting_for_character)
